...
def _resolution_for_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "RULE_PERIOD_OVERLAP":
        if bool(conflict.get("can_auto_solve")):
            return {
                "can_solve": True,
    ...
    if kind in {"FK_VIOLATION", "RULE_PERIOD_SHORTER"} or error_type in {"FK_MISSING", "FK_AMBIGUOUS"}:
        return {
            "can_solve": False,
            "solve_mode": "MANUAL_REQUIRED",
            "label": "Tidak bisa auto-solve",
...
def _decorate_rejected_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **row,
        "resolution": _resolution_for_row(row),
        "conflict": {
            "incoming": incoming,
            "existing": existing,
        },
...
def _apply_solve_for_row(row: dict[str, Any]) -> None:
    ...
    payload = row.get("raw_payload") or {}
    conflict = payload.get("__conflict") if isinstance(payload, dict) else None
    if not isinstance(conflict, dict) or conflict.get("kind") != "RULE_PERIOD_OVERLAP":
...
def rerun_batch(batch_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        new_batch_id = create_rerun_batch(batch_id)
    except ValueError:
        raise HTTPException(status_code=404, detail="Batch not found")

    background_tasks.add_task(run_ingestion_flow, new_batch_id)
    return {
...
def solve_rejected(
    ...
    deleted = delete_rejected_row(batch_id, rejected_id)
    if not deleted:
        raise HTTPException(status_code=500, detail="Failed to mark rejected row as solved")

    metrics = _refresh_batch_metrics_after_solve(batch_id)
    queued = False
    rerun_batch_id = None
    if rerun:
        rerun_batch_id = create_rerun_batch(batch_id)
        background_tasks.add_task(run_ingestion_flow, rerun_batch_id)
        queued = True
