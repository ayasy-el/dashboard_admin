  ...
  return (
    ...
    >
      {stats.map((stat) => {
        ...
        return (
          <Card className="@container/card border-border/70" key={stat.id}>
            <CardHeader>
              <CardDescription className="text-[11px] font-semibold uppercase tracking-wide">{stat.label}</CardDescription>
              <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
                {formatNumber(stat.currentTotal)}
              </CardTitle>
              <CardAction>
                <Badge variant="outline" className="rounded-full border-border/70 bg-white/80 text-[11px]">
                  <TrendIcon />
                  {isPositive ? "+" : ""}
